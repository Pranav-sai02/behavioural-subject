...
export class BodyLocationsState {
  ...
  load(ctx: StateContext<BodyLocationsStateModel>) {
    return this.bodyLocationsService.getBodyLocations().pipe(
      tap((items) => {
        const sorted = [...items].sort((a, b) => (b.BodyLocationId ?? 0) - (a.BodyLocationId ?? 0));
  ...
  addRow(ctx: StateContext<BodyLocationsStateModel>, action: AddBodyLocationRowLocally) {
    const state = ctx.getState();
    ctx.patchState({ bodyLocations: [action.payload, ...state.bodyLocations] });
  }

  @Action(UpdateBodyLocation)
  update(ctx: StateContext<BodyLocationsStateModel>, action: UpdateBodyLocation) {
    const state = ctx.getState();
  ...
  softDelete(ctx: StateContext<BodyLocationsStateModel>, action: SoftDeleteBodyLocation) {
    const state = ctx.getState();
    const updated = state.bodyLocations.filter((b) => b.BodyLocationId !== action.payload.BodyLocationId);
    ctx.patchState({ bodyLocations: updated });

    return this.bodyLocationsService.softDeleteBodyLocation(action.payload.BodyLocationId).pipe(
      catchError((error) => {
