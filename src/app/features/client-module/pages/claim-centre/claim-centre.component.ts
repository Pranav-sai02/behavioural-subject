...
@Component({
  ...
  styleUrls: ['./claim-centre.component.css'],
})
export class ClaimCentreComponent implements OnInit {
  ...
  onAddClaimCentre(): void {
    ...
    const alreadyExists = this.rowData.some((c) => c.ClaimCentreId === selectedId);
    if (alreadyExists) {
    ...
    const selectedCentre = this.claimCentreList.find((c) => c.id === selectedId);
    if (selectedCentre) {
  ...
  softDelete(row: ClientClaimCentre): void {
    ...
    const updatedList = this.rowData.filter((c) => c.ClaimCentreId !== row.ClaimCentreId);
